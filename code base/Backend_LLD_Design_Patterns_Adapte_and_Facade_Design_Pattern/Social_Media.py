...
Review the existing APIs: Study the APIs and data formats of the social media platforms you need to integrate. Understand the differences in their APIs and how they interact with their respective systems. You will find the external API classes in the services.py file and look at the FacebookApi and TwitterApi classes.

Implement the adapter interface: You have been provided with a SocialMediaAdapter interface. Your task is to add common methods of the APIs to the interface, implement this interface in different adapter classes, each corresponding to a different social media platform. The adapters should adapt the provider-specific APIs into a format that matches the SocialMediaAdapter interface.
...
Instructions
Add methods to the SocialMediaAdapter interface that match the common functionality of the social media platforms. You will implement this interface in the adapter classes for Twitter and Facebook. These classes should adapt the platform-specific APIs into a common format.
Implement the Adapter pattern by creating adapter classes that implement the SocialMediaAdapter interface and adapt the APIs of different social media platforms. The classes TwitterAdapter and FacebookAdapter are provided as placeholders. You need to complete these classes to adapt the Twitter and Facebook APIs to the common SocialMediaAdapter interface.
Run the provided test cases in the SocialMediaAdapterTest class to verify the correctness of your adapter pattern implementation. Make sure your adapters have the expected methods and interact with the platform APIs as required.
'''