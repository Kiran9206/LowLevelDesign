...
Instructions
In the BaseApiDecorator class, inherit from the Api class and implement the method execute_request() to delegate the API endpoint operations to the wrapped API object. Remember to set the api instance variable in the constructor.
Implement the LoggingDecorator and RateLimitDecorator classes by extending the BaseApiDecorator class and adding the desired functionalities to the API endpoints.
For each decorator class, implement the necessary logic to add the desired functionality, such as logging incoming requests or rate-limiting the API endpoints. You can use the utility methods provided in the ApiUtils class for this purpose.
