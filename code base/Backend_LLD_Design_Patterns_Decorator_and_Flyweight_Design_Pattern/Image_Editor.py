...
Modify the BaseImageDecorator class to extend the ImageEditor class and act as the base class for all decorator classes. The BaseImageDecorator class should have a constructor that takes an ImageEditor object as a parameter and sets it as an instance variable image_editor.
...
Instructions
In the BaseImageDecorator class, inherit from the ImageEditor class and implement the render() method to delegate the rendering operation to the wrapped ImageEditor object. Remember to set the image_editor instance variable in the constructor.
Implement the BlurImageDecorator, SharpenImageDecorator, and GrayscaleImageDecorator classes by extending the BaseImageDecorator class and applying the respective filters to the image.
For each decorator class, call the relevant methods from the ImageUtils class to apply the filter to the image, such as apply_blur(), apply_sharpen(), and apply_grayscale().
Run the provided test cases in the TestImageDecorator class to verify the correctness of your implementation. You are not required to edit the test cases themselves.
'''